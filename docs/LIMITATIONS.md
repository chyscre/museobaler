...
| Prefetching the tour when the desk clears a visitor | Deliberately not implemented. Pulling every exhibit's picture and a narration per language at clearance would use about 1.3 MB an exhibit, or over 10 MB for the current eight, exactly when a queue is forming on the museum's single uplink. It would also spend storage on halls a visitor may never walk into, even though browser storage can later be evicted. Media is fetched through temporary signed URLs and cached on first view instead, so offline support covers the exhibits actually visited without weakening the admission boundary. |
