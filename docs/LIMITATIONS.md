...
| Prefetching the tour when the desk clears a visitor | It would pull every exhibit's picture and a narration per language the moment admission is granted - about 1.3 MB an exhibit, so over 10 MB for the current eight, and more as the collection grows. That lands on the museum's single uplink exactly when a queue is forming at the counter, and on the cheapest phone in the queue it is 10 MB of storage the browser will evict anyway (see "Offline works within what a phone has already seen"), spent on halls the visitor may never walk into. Media is cached on first view instead, which costs the same bytes only for what is actually looked at, and spreads them across the visit rather than stacking them at the door. |
